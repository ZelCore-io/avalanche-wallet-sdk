 ...
 */
export async function avmGetAtomicUTXOs(addrs, sourceChain) {
...
export async function platformGetAtomicUTXOs(addrs, sourceChain) {
...
// todo: Use end index to get ALL utxos
export async function evmGetAtomicUTXOs(addrs, sourceChain) {
    if (addrs.length > 1024) {
        throw new Error('Number of addresses can not be greater than 1024.');
    }
    const sourceChainId = chainIdFromAlias(sourceChain);
    let result = (await cChain.getUTXOs(addrs, sourceChainId)).utxos;
    return result;
}
export async function getStakeForAddresses(addrs) {
    if (addrs.length <= 256) {
...
export async function avmGetAllUTXOs(addrs) {
    if (addrs.length <= 1024) {
...
export async function avmGetAllUTXOsForAddresses(addrs, endIndex) {
    if (addrs.length > 1024)
...
export async function platformGetAllUTXOs(addrs) {
    if (addrs.length <= 1024) {
...
export async function platformGetAllUTXOsForAddresses(addrs, endIndex) {
    let response;
    if (!endIndex) {
