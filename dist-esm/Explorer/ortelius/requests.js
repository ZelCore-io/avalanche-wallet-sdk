 ...
 */
export async function getAddressHistoryEVM(addr) {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
    let endpoint = `v2/ctransactions?address=${addr}`;
    let data = (await explorer_api.get(endpoint)).Transactions;
    data.sort((a, b) => {
...
export async function getTx(txID) {
    ...
    return await explorer_api.get(url);
}
/**
 * Returns ortelius data for a transaction hash on C chain EVM,
 * @param txHash
 */
export async function getTxEvm(txHash) {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
    let endpoint = `v2/ctransactions?hash=${txHash}`;
    let data = (await explorer_api.get(endpoint)).Transactions[0];
    return data;
}
/**
 ...
 * @param endTime
 */
export async function getAddressHistory(addrs, limit = 20, chainID, endTime) {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
 ...
 * @param addrs
 */
export async function getAddressChains(addrs) {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
    // Strip the prefix
...
export async function getAddressDetailX(addr) {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
